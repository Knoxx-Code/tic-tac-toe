...
def createBoard(board):
    for row in range(7):
        # create Lists that will hold the rows
        board_rows = []
...
# Function that checks if a player has won. Updated to check for four consecutive cells with same symbols.
...
# Minimax algorithm modified by adding the alpha-beta heuristic
def alphabeta(board, depth, maximizing_player, player,alpha,beta):
    # Return utility value of player
    if isEnd(board) or depth==0:
        return utility(board, player)

    # If current player(AI) is maximizing
    if maximizing_player:
        ...
        for move in ai_possible_moves(board):
            ...
            # opponents turn. It is called with depth-1 since we are specifying the depth and need to terminate the
            # search when depth reaches 0
            score = alphabeta(board_copy, depth - 1, False, player,alpha,beta)
            # Get the max score and update best score accordingly
            best_score = max(best_score, score)
            # alpha is the best maximum score found by the maximizing player
        ...
        return best_score
    # If current player(AI) is minimizing
    else:
        best_score = 1000
        for move in ai_possible_moves(board):
            ...
            # opponents turn.It is called with depth-1 since we are specifying the depth and need to terminate
            # the search when depth reaches 0
            score = alphabeta(board_copy, depth - 1, True, player,alpha, beta)
            # Get the min score and update best score
            best_score = min(best_score, score)
            # beta is the best minimum score found by the minimizing player
...
# Modified Function to choose which move the AI makes based on the minimax algorithm
def ai_make_move(board, player,depth):
    best_score = -1000
    best_move = None
    # Set the initial values for alpha and beta
    alpha = -math.inf
    beta = math.inf
    ...
    for move in ai_possible_moves(board):
        ...
        # Call the alphabeta function to evaluate the score
        score = alphabeta(board_copy, depth - 1, False, player,alpha, beta)
...
def play(board):
    print("----------TIC-TAC-TOE 7X7-----------")

    # Get the desired depth to which the AI will reach in the tree
    depth = int(input("Enter desired depth for AI: "))
    ...
    while not isEnd(board):
        ...
        # If player is AI
            ...
            print(f"--AI'S TURN--")
            # Get row and column of AI
            row, column = ai_make_move(board, player,depth)
